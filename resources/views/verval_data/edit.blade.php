...
@section('content')
    <div class="content-body" style="padding: 24px;">
        ...
        @if ($errors->any())
            <div
                style="background: #fee2e2; border-left: 4px solid #ef4444; color: #b91c1c; padding: 16px; margin-bottom: 24px; border-radius: 4px;">
        ...
        <form action="{{ route('data-verval.update', $vervalData->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-section">
                <h4><i class="fas fa-user-edit"></i> Data Pribadi Tambahan & Lahan</h4>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="luas_tanah" class="form-control"
                            value="{{ old('luas_tanah', $vervalData->luas_tanah) }}" placeholder="Contoh: 100 m2">
                    </div>
                    <div class="form-group">
                        <label>Telah Ditempati Selama (Tahun/Bulan)</label>
            ...
            <div class="form-section">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px;">
                    <div class="form-group">
                        ...
                        @if($vervalData->ktp)
                            <small style="color: green; margin-top: 4px; display: block;"><i class="fas fa-check"></i> Sudah
                                diunggah</small>
                        @endif
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <select name="jenis_kepemilikan_lahan" class="form-select">
                            ...
                            <option value="SHM" {{ $vervalData->jenis_kepemilikan_lahan == 'SHM' ? 'selected' : '' }}>SHM
                                (Sertifikat Hak Milik)</option>
                            <option value="SHGB" {{ $vervalData->jenis_kepemilikan_lahan == 'SHGB' ? 'selected' : '' }}>SHGB
                                (Sertifikat Hak Guna Bangunan)</option>
                            <option value="Girik/Letter C" {{ $vervalData->jenis_kepemilikan_lahan == 'Girik/Letter C' ? 'selected' : '' }}>Girik / Letter C</option>
                            <option value="SKT" {{ $vervalData->jenis_kepemilikan_lahan == 'SKT' ? 'selected' : '' }}>SKT
                                (Surat Keterangan Tanah)</option>
                            <option value="AJB" {{ $vervalData->jenis_kepemilikan_lahan == 'AJB' ? 'selected' : '' }}>AJB
                                (Akta Jual Beli)</option>
                            ...
                            <option value="Lainnya" {{ $vervalData->jenis_kepemilikan_lahan == 'Lainnya' ? 'selected' : '' }}>
                                Lainnya</option>
                        </select>
                    </div>
                    <div class="form-group">
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px;">
                    <div class="photo-box">
                        ...
                        @if($vervalData->foto_sudut_depan)
                            <small style="color: green; margin-top: 8px; display: block;"><i class="fas fa-check"></i> Gambar
                                tersimpan</small>
                        @endif
                    </div>
                    <div class="photo-box">
                    ...
                    <div
                        style="grid-column: span 3; display: grid; grid-template-columns: 1fr 1fr; gap: 24px; max-width: 800px; margin: 0 auto; width: 100%;">
                        <div class="photo-box">
                            <label>Sudut Kiri</label>
                            <input type="file" name="foto_sudut_kiri" class="form-control" accept="image/*">
                            @if($vervalData->foto_sudut_kiri)
                                <small style="color: green; margin-top: 8px; display: block;"><i class="fas fa-check"></i>
                                    Gambar tersimpan</small>
                            @endif
                        </div>
                        <div class="photo-box">
            ...
            <div class="form-section">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px;">
                    <div class="form-group">
                        ...
                        <input type="text" id="disp_latitude" class="form-control" value="{{ $vervalData->latitude }}"
                            disabled placeholder="Menunggu GPS...">
                ...
                <small style="color: var(--text-muted);"><i class="fas fa-info-circle"></i> Koordinat lokasi akan terisi
                    otomatis menggunakan GPS perangkat/browser Anda.</small>
            </div>

            <div style="text-align: right; padding-bottom: 40px;">
                <a href="{{ route('data-verval') }}" class="btn btn-outline"
                    style="margin-right: 12px; padding: 12px 24px; border-radius: 8px;">Batal</a>
                <button type="submit" class="btn btn-primary"
                    style="padding: 12px 24px; font-weight: 700; border-radius: 8px;">
    ...
    <div class="modal-overlay" id="gpsModal">
        <div class="modal-box" style="max-width: 440px;">
            ...
            <div class="modal-body" style="padding: 24px; text-align: center;">
                <div
                    style="width: 60px; height: 60px; border-radius: 50%; background: rgba(220, 53, 69, 0.1); color: #dc3545; display: inline-flex; align-items: center; justify-content: center; font-size: 24px; margin: 0 auto 16px;">
                    <i class="fas fa-map-marker-alt"></i>
                </div>
                <p style="font-size: 14px; margin-bottom: 0;">Anda harus mengaktifkan dan mengizinkan akses GPS (Lokasi)
                    pada perangkat/browser Anda untuk dapat mengisi data verval ini.</p>
                <p style="font-size: 13px; color: var(--text-muted); margin-top: 12px;">Silakan izinkan akses lokasi saat
                    diminta oleh browser.</p>
            </div>

            <div class="modal-footer"
                style="padding: 16px 20px; background: var(--bg-body); border-top: 1px solid rgba(0, 40, 85, 0.06); display: flex; justify-content: center;">
