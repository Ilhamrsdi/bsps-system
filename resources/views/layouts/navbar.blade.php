<header class="navbar">
    <div class="navbar-left">
        ...
        <div class="page-title">
            <h2>@yield('title_header', 'Dashboard PUPR')</h2>
        </div>
    </div>

    <div class="navbar-right">
        <form action="{{ url('/data-mingguan') }}" method="GET" class="search-box" style="margin:0;">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari kegiatan, lokasi, petugas..." />
        </form>
        ...
        <div class="user-profile-wrapper" id="notifDropdownWrapper" style="position:relative;">
            <button class="nav-btn" id="notifBtnToggle" title="Notifikasi Petugas" type="button" style="position:relative;">
            ...
            <div class="user-dropdown-menu" id="notifMenuBox" style="width:320px;right:0;left:auto;padding:0;overflow:hidden;">
                <div class="user-dropdown-header" style="background:var(--primary);color:#fff;padding:14px 18px;text-align:center;">
                    <div style="font-weight:700;font-size:14px;"><i class="fas fa-bell" style="margin-right:6px;"></i> Notifikasi Survei</div>
                </div>
                <div style="max-height:280px;overflow-y:auto;background:var(--bg-card);">
                    @forelse($recentNavSurveys as $n)
                        <a href="{{ url('/setting') }}" style="display:flex;align-items:flex-start;gap:12px;padding:12px 16px;border-bottom:1px solid rgba(0,40,85,0.06);text-decoration:none;color:inherit;transition:var(--transition);">
                            ...
                            <div style="flex:1;min-width:0;">
                                ...
                                <div style="font-size:11.5px;color:var(--text-secondary);margin-top:2px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">
                                    Mengisi survei: {{ $n->nama_kegiatan ?: ($n->dataMingguan->nama_kegiatan ?? 'Kegiatan Lapangan') }}
                                </div>
                                <div style="font-size:10px;color:var(--text-muted);margin-top:4px;">
                                    <i class="fas fa-clock"></i> {{ $n->updated_at ? $n->updated_at->diffForHumans() : 'Baru saja' }}
                                </div>
                            </div>
                        </a>
                    @empty
                ...
                <a href="{{ url('/setting') }}" style="display:block;text-align:center;padding:10px;font-size:12px;font-weight:700;color:var(--primary);background:var(--bg-body);text-decoration:none;border-top:1px solid rgba(0,40,85,0.06);">
